...
<Tip>
**One client for every surface.** The same "Web application" OAuth client works for the desktop app, the WebUI, and the headless server — they all hand the OAuth round-trip to the hosted relay at `agents.craft.do/auth/callback`, which routes the result back to the right destination.
</Tip>
