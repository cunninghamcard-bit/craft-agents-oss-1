...
### Tier 3 — Remote OAuth (not supported)

GitHub's remote MCP endpoint at `https://api.githubcopilot.com/mcp/` requires OAuth with a pre-registered client application. **Craft Agents is not registered with GitHub as an OAuth client, so this will not work.**

Do not attempt remote OAuth setup — it will fail during the authorization step. Use Tier 1 or Tier 2 instead.
