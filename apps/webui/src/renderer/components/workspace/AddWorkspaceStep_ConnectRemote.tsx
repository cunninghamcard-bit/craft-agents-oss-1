...
async function resolveUniqueSlug(baseName: string): Promise<{ slug: string; path: string }> {
  ...
  while (true) {
    const result = await window.webAgentAPI.checkWorkspaceSlug(slug)
    if (!result.exists) {
...
export function AddWorkspaceStep_ConnectRemote({
  ...
  useEffect(() => {
    window.webAgentAPI.getHomeDir().then(setHomeDir)
  }, [])
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    setTestError(null)
    try {
      const result = await window.webAgentAPI.testRemoteConnection(serverUrl, token)
  ...
  const handleConnect = useCallback(async () => {
    ...
    if (isCreateNew || isFreshServer) {
      ...
      try {
        const created = await window.webAgentAPI.invokeOnServer(
