...
export function useInlineMention({
  ...
  const handleInputChange = React.useCallback((value: string, cursorPosition: number) => {
    ...
    if (isValidTrigger) {
      ...
      window.webAgentAPI.debugLog('[mention] filterText:', filterText, 'basePath:', basePath, 'cacheSize:', fileCache.current.length)
      if (basePath && filterText.length >= 1) {
        if (fileCache.current.length > 0) {
          ...
          window.webAgentAPI.debugLog('[mention] cache hit:', filtered.length, 'items')
          ...
          fileSearchTimeout.current = setTimeout(async () => {
            try {
              window.webAgentAPI.debugLog('[mention] calling IPC searchFiles:', basePath, filterText)
              const results = await window.webAgentAPI.searchFiles(basePath, filterText)
              window.webAgentAPI.debugLog('[mention] IPC returned:', results?.length, 'results')
              fileCache.current = results
              const filtered = filterCacheResults(fileCache.current, filterText)
              window.webAgentAPI.debugLog('[mention] after cache filter:', filtered.length, 'items')
              setFileResults(filtered)
              setCommittedFilter(filterText)
            } catch (err) {
              window.webAgentAPI.debugLog('[mention] IPC searchFiles error:', String(err))
            }
          }, 150)
        }
      } else {
        window.webAgentAPI.debugLog('[mention] skipping file search (no basePath or empty filter)')
        if (fileSearchTimeout.current) {
          clearTimeout(fileSearchTimeout.current)
          fileSearchTimeout.current = null
        }
