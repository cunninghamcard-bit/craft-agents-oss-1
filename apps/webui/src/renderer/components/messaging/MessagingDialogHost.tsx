...
export function MessagingDialogHost() {
  ...
  React.useEffect(() => {
    ...
    const off = window.webAgentAPI.onMessagingBindingChanged(async () => {
      const current = stateRef.current
      if (current.kind !== 'pairing' || current.code === null) return
      try {
        const bindings = await window.webAgentAPI.getMessagingBindings()
  ...
  const openPairing = async (sessionId: string, platform: 'telegram' | 'whatsapp') => {
    ...
    try {
      const result = await window.webAgentAPI.generateMessagingPairingCode(sessionId, platform)
      setState({
        kind: 'pairing',
        platform,
        sessionId,
