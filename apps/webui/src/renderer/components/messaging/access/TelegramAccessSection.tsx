...
export function TelegramAccessSection({ workspaceId, accessMode, onAccessModeChange }: Props) {
  ...
  const loadAll = React.useCallback(async () => {
    const [o, p] = await Promise.all([
      window.webAgentAPI.getMessagingPlatformOwners('telegram').catch(() => []),
      window.webAgentAPI.getMessagingPendingSenders('telegram').catch(() => []),
    ])
    setOwners(o)
    setPending(p)
  }, [])
  ...
  React.useEffect(() => {
    void loadAll()
    const offBinding = window.webAgentAPI.onMessagingBindingChanged((wsId) => {
      if (wsId === workspaceId) void loadAll()
    })
    const offPending = window.webAgentAPI.onMessagingPendingChanged?.((wsId) => {
      if (wsId === workspaceId) void loadAll()
    })
    return () => {
      offBinding()
  ...
  const handleLockDown = async () => {
    try {
      await window.webAgentAPI.setMessagingPlatformAccessMode('telegram', 'owner-only')
  ...
  const handleRemoveOwner = async (userId: string) => {
    ...
    try {
      await window.webAgentAPI.setMessagingPlatformOwners('telegram', next)
      setOwners(next)
    } catch (err) {
  ...
  const handleAllow = async (sender: PendingSender) => {
    try {
      ...
      const result = await window.webAgentAPI.allowMessagingPendingSender(
        'telegram',
        sender.userId,
        entryKey,
      )
  ...
  const handleIgnore = async (sender: PendingSender) => {
    try {
      await window.webAgentAPI.dismissMessagingPendingSender(
        'telegram',
        sender.userId,
        {
          ...(sender.reason ? { reason: sender.reason } : {}),
