...
const roleHandlers: Record<string, () => void> = {
  undo: () => window.webAgentAPI.menuUndo(),
  redo: () => window.webAgentAPI.menuRedo(),
  cut: () => window.webAgentAPI.menuCut(),
  copy: () => window.webAgentAPI.menuCopy(),
  paste: () => window.webAgentAPI.menuPaste(),
  selectAll: () => window.webAgentAPI.menuSelectAll(),
  zoomIn: () => window.webAgentAPI.menuZoomIn(),
  zoomOut: () => window.webAgentAPI.menuZoomOut(),
  resetZoom: () => window.webAgentAPI.menuZoomReset(),
  minimize: () => window.webAgentAPI.menuMinimize(),
  zoom: () => window.webAgentAPI.menuMaximize(),
}
...
function renderSubmenuItem(
  ...
  if (item.type === 'url') {
    ...
    return (
      <StyledDropdownMenuItem key={item.id} onClick={() => window.webAgentAPI.openUrl(item.url)}>
...
export function DesktopAppMenu({
  ...
  useEffect(() => {
    window.webAgentAPI.isDebugMode().then(setIsDebugMode)
  }, [])
  ...
  return (
    <DropdownMenu>
      ...
      <StyledDropdownMenuContent align="start" minWidth="min-w-48">
          ...
          <StyledDropdownMenuSubContent>
            {HELP_LINKS.map((link) => {
              ...
              return (
                <StyledDropdownMenuItem
                  key={link.id}
                  onClick={() => window.webAgentAPI.openUrl(link.url)}
                >
                  {Icon && <Icon className="h-3.5 w-3.5" />}
                  {t(link.labelKey)}
                  <Icons.ExternalLink className="h-3 w-3 ml-auto text-muted-foreground" />
        ...
        <StyledDropdownMenuItem onClick={() => window.webAgentAPI.menuQuit()}>
...
/** Renders the Debug submenu by mapping over `DEBUG_MENU.items`. */
...
const debugHandlers: Record<string, () => void> = {
  toggleDevTools: () => window.webAgentAPI.menuToggleDevTools(),
}
