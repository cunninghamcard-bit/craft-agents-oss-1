...
/** True when running in web UI (browser) rather than WebUI. */
const isWebMode = window.webAgentAPI.getRuntimeEnvironment() === 'web'
 ...
 *
 * In WebUI: loads via the custom thumbnail:// protocol (efficient 64x64 resize).
...
const FileThumbnail = memo(function FileThumbnail({ file }: { file: SessionFile }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    window.webAgentAPI.readFilePreviewDataUrl(file.path, 64).then((url) => {
...
export function SessionFilesSection({ sessionId, className, sessionFolderPath, hideHeader = false }: SessionFilesSectionProps) {
  ...
  const loadFiles = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const sessionFiles = await window.webAgentAPI.getSessionFiles(sessionId)
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      void window.webAgentAPI.watchSessionFiles(sessionId)

      // Listen for file change events
      const unsubscribe = window.webAgentAPI.onSessionFilesChanged((changedSessionId) => {
        if (changedSessionId === sessionId && mountedRef.current) {
          void loadFiles()
        }
      })

      const unsubscribeReconnect = window.webAgentAPI.onReconnected(() => {
      ...
      return () => {
        ...
        void window.webAgentAPI.unwatchSessionFiles()
      }
    }
  ...
  const handleRevealInFileManager = useCallback((path: string) => {
    window.webAgentAPI.showInFolder(path)
  }, [])
  ...
  const handleFileClick = useCallback((file: SessionFile) => {
    if (file.type === 'directory') {
      ...
      window.webAgentAPI.openFile(file.path)
    } else {
      ...
      {!hideHeader && (
        <div className="flex items-center justify-between px-4 pt-4 pb-2 shrink-0 select-none">
          ...
          {sessionFolderPath && (
            <button
              type="button"
              onClick={() => window.webAgentAPI.showInFolder(sessionFolderPath)}
