...
export function SendResourceToWorkspaceDialog({
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const { bundle, warnings: exportWarnings } = await window.webAgentAPI.exportResources(
        activeWorkspaceId,
        exportOptions,
      )
      ...
      if (targetWorkspace.remoteServer) {
        ...
        importResult = await window.webAgentAPI.invokeOnServer(
          url, token,
          'resources:import',
        ...
        importResult = await window.webAgentAPI.importResources(
          selectedWorkspaceId,
          bundle,
          mode,
        )
