...
export function SendToWorkspaceDialog({
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onTransferProgress((p) => {
  ...
  const handleTransfer = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < sessionIds.length; i++) {
        ...
        const result = await window.webAgentAPI.transferSessionToWorkspace(sessionId, selectedWorkspaceId, i, sessionIds.length)
