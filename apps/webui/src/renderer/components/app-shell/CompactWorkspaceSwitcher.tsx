...
export function CompactWorkspaceSwitcher({
  ...
  const checkRemoteHealth = useCallback(() => {
    ...
    for (const ws of remoteWorkspaces) {
      window.webAgentAPI.testRemoteConnection(ws.remoteServer!.url, ws.remoteServer!.token)
        .then(result => {
  ...
  const handleRemoveWorkspace = useCallback(async (workspace: Workspace) => {
    ...
    const removed = await window.webAgentAPI.removeWorkspace(workspace.id)
    if (removed) {
  ...
  const handleReconnectWorkspace = useCallback(async (workspaceId: string, remoteServer: { url: string; token: string; remoteWorkspaceId: string }) => {
    await window.webAgentAPI.updateWorkspaceRemoteServer(workspaceId, remoteServer)

    if (workspaceId === activeWorkspaceId) {
      await window.webAgentAPI.reconnectTransport()
      await waitForTransportConnected(window.webAgentAPI)
    } else {
