...
export function ServerDirectoryBrowser({
  ...
  const navigateTo = useCallback(async (dirPath: string) => {
    ...
    try {
      const result = await window.webAgentAPI.listServerDirectory(dirPath)
      setListing(result)
    ...
    const init = async () => {
      if (mode === 'browse') {
        ...
        const tryNavigate = async (path: string) => {
          const result = await window.webAgentAPI.listServerDirectory(path)
          setListing(result)
          setPathInput(result.currentPath)
        ...
        try {
          if (initialPath) {
            ...
            try {
              const serverHome = await window.webAgentAPI.getServerHomeDir()
        ...
        const homeDir = await window.webAgentAPI.getHomeDir()
