...
export default function SourceInfoPage({ sourceSlug, workspaceId, onDelete }: SourceInfoPageProps) {
  ...
  useEffect(() => {
    ...
    const loadSource = async () => {
      try {
        const sources = await window.webAgentAPI.getSources(workspaceId)
        ...
        if (found) {
          ...
          const config = await window.webAgentAPI.getSourcePermissionsConfig(workspaceId, sourceSlug)
          if (isMounted) {
  ...
  useEffect(() => {
    ...
    const loadTools = async () => {
      try {
        const result = await window.webAgentAPI.getMcpTools(workspaceId, sourceSlug)
        if (!isMounted) return
  ...
  useEffect(() => {
    ...
    window.webAgentAPI.getWorkspaceSettings(workspaceId).then((settings) => {
      if (settings) {
  ...
  useEffect(() => {
    if (!window.webAgentAPI?.onSourcesChanged) return

    const cleanup = window.webAgentAPI.onSourcesChanged((changedWorkspaceId, sources) => {
  ...
  const handleOpenUrl = useCallback(async () => {
    ...
    if (window.webAgentAPI) {
      if (sourceUrl.startsWith('http://') || sourceUrl.startsWith('https://')) {
        await window.webAgentAPI.openUrl(sourceUrl)
      } else {
        await window.webAgentAPI.showInFolder(sourceUrl)
      }
    }
  }, [source, sourceUrl])
  ...
  const handleOpenSourceFolder = useCallback(async () => {
    if (!source) return
    if (window.webAgentAPI) {
      await window.webAgentAPI.showInFolder(source.folderPath)
    }
  }, [source])
  ...
  const handleDelete = useCallback(async () => {
    if (!source) return
    try {
      await window.webAgentAPI.deleteSource(workspaceId, sourceSlug)
  ...
  const handleOpenInNewWindow = useCallback(() => {
    window.webAgentAPI.openUrl(`craftagents://sources/source/${sourceSlug}?window=focused`)
  }, [sourceSlug])
