...
export default function PermissionsSettingsPage() {
  ...
  useEffect(() => {
    const loadPermissions = async () => {
      ...
      try {
        ...
        const { config: defaults, path: defaultsPath } = await window.webAgentAPI.getDefaultPermissionsConfig()
        ...
        if (activeWorkspaceId) {
          const workspace = await window.webAgentAPI.getWorkspacePermissionsConfig(activeWorkspaceId)
  ...
  useEffect(() => {
    if (!window.webAgentAPI?.onDefaultPermissionsChanged) return

    const unsubscribe = window.webAgentAPI.onDefaultPermissionsChanged(async () => {
      // Reload default permissions when the file changes
      const { config: defaults } = await window.webAgentAPI.getDefaultPermissionsConfig()
      setDefaultConfig(defaults)
    })
                    ...
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        ...
                        <p>
                          <button
                            type="button"
                            onClick={() => window.webAgentAPI?.openUrl(getDocUrl('permissions'))}
