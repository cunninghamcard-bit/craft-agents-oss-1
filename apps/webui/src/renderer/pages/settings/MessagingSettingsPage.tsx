...
export default function MessagingSettingsPage() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        const rows = await window.webAgentAPI.getMessagingBindings()
    ...
    load()
    const off = window.webAgentAPI.onMessagingBindingChanged((wsId) => {
...
function PlatformRow({ platform, workspaceId }: { platform: Platform; workspaceId: string }) {
  ...
  const refreshSupergroup = React.useCallback(async () => {
    ...
    try {
      const sg = await window.webAgentAPI.getMessagingSupergroup()
  ...
  const refreshTelegramAccessMode = React.useCallback(async () => {
    if (platform !== 'telegram') return
    try {
      const mode = await window.webAgentAPI.getMessagingPlatformAccessMode('telegram')
  ...
  React.useEffect(() => {
    let cancelled = false
    window.webAgentAPI.getMessagingConfig().then((cfg) => {
      if (cancelled) return
      const next = cfg?.runtime?.[platform]
      setRuntime((next ?? defaultRuntime(platform)) as MessagingPlatformRuntimeInfo)
    })
    const off = window.webAgentAPI.onMessagingPlatformStatus((wsId, p, status) => {
  ...
  const handleUnlock = async () => {
    try {
      await window.webAgentAPI.setMessagingPlatformAccessMode('telegram', 'open')
  ...
  const handleDisconnect = async () => {
    try {
      await window.webAgentAPI.disconnectMessagingPlatform(platform)
      toast.success(
  ...
  const handleForget = async () => {
    try {
      await window.webAgentAPI.forgetMessagingPlatform(platform)
      toast.success(
        t(`settings.messaging.${platform}.disconnected`, {
          defaultValue: 'Disconnected',
        }),
  ...
  const handleUnbind = async (binding: MessagingBinding) => {
    try {
      await window.webAgentAPI.unbindMessagingBinding(binding.id)
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t('common.error'))
    }
  }
          ...
          <>
            ...
            <TelegramBindingsBody
              ...
              onUnpairSupergroup={async () => {
                try {
                  await window.webAgentAPI.unbindMessagingSupergroup()
                  toast.success(
...
function TelegramBindingsBody({
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        const owners = await window.webAgentAPI.getMessagingPlatformOwners('telegram')
  ...
  const handleAccessChange = React.useCallback(
    async (bindingId: string, next: BindingAccess) => {
      try {
        await window.webAgentAPI.setMessagingBindingAccess(bindingId, {
