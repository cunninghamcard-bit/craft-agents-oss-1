 ...
 * ServerSettingsPage
 *
 * Configure the WebUI app to act as a remote server,
...
export default function ServerSettingsPage() {
  ...
  const loadSettings = useCallback(async () => {
    try {
      const [config, serverStatus] = await Promise.all([
        window.webAgentAPI.getServerConfig(),
        window.webAgentAPI.getServerStatus(),
      ])
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true)
    try {
      await window.webAgentAPI.setServerConfig(formToConfig(form))
      setSavedForm(form)
      const newStatus = await window.webAgentAPI.getServerStatus()
      setStatus(newStatus)
  ...
  const handleBrowseCert = async () => {
    const paths = await window.webAgentAPI.openFileDialog()
    if (paths.length > 0) {
            ...
            {needsRestart && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-warning/10 border border-warning/20 text-xs text-warning">
                ...
                <Button
                  ...
                  onClick={() => window.webAgentAPI.relaunchApp()}
                >
