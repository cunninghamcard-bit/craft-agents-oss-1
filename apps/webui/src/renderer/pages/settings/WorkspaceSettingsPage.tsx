...
export default function WorkspaceSettingsPage() {
  ...
  useEffect(() => {
    const loadWorkspaceSettings = async () => {
      if (!window.webAgentAPI || !activeWorkspaceId) {
      ...
      try {
        const settings = await window.webAgentAPI.getWorkspaceSettings(activeWorkspaceId)
        if (settings) {
          ...
          const sources = await window.webAgentAPI.getSources(activeWorkspaceId)
          ...
          if (healedSlugs.length !== savedSlugs.length) {
            window.webAgentAPI.updateWorkspaceSetting(activeWorkspaceId, 'enabledSourceSlugs', healedSlugs)
          }
        }
        ...
        for (const ext of ICON_EXTENSIONS) {
          try {
            const iconData = await window.webAgentAPI.readWorkspaceImage(activeWorkspaceId, `./icon.${ext}`)
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onSourcesChanged((workspaceId: string, sources: LoadedSource[]) => {
      ...
      setEnabledSourceSlugs(prev => {
        ...
        if (healed.length !== prev.length && activeWorkspaceId) {
          window.webAgentAPI.updateWorkspaceSetting(activeWorkspaceId, 'enabledSourceSlugs', healed)
        }
        return healed
      })
    })
  ...
  const updateWorkspaceSetting = useCallback(
    async <K extends keyof WorkspaceSettings>(key: K, value: WorkspaceSettings[K]) => {
      if (!window.webAgentAPI || !activeWorkspaceId) return false

      try {
        await window.webAgentAPI.updateWorkspaceSetting(activeWorkspaceId, key, value)
        return true
      } catch (error) {
  ...
  const handleIconUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !activeWorkspaceId || !window.webAgentAPI) return
    ...
    try {
      ...
      // Upload to workspace
      await window.webAgentAPI.writeWorkspaceImage(activeWorkspaceId, `./icon.${ext}`, base64, file.type)
