...
export default function PreferencesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const result = await window.webAgentAPI.readPreferences()
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const result = await window.webAgentAPI.writePreferences(json)
        if (result.success) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (lastSavedRef.current !== currentJson && !isInitialLoadRef.current) {
        ...
        window.webAgentAPI.writePreferences(currentJson).catch((err) => {
