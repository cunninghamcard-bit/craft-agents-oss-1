...
export default function SkillInfoPage({ skillSlug, workspaceId, workingDirectory }: SkillInfoPageProps) {
  ...
  useEffect(() => {
    ...
    const loadSkill = async () => {
      try {
        const skills = await window.webAgentAPI.getSkills(workspaceId, workingDirectory)
    ...
    const unsubscribe = window.webAgentAPI.onSkillsChanged?.((changedWorkspaceId, skills) => {
  ...
  const handleOpenInFinder = useCallback(async () => {
    ...
    try {
      await window.webAgentAPI.showInFolder(skill.path)
    } catch (err) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      await window.webAgentAPI.deleteSkill(workspaceId, skillSlug)
  ...
  const handleOpenInNewWindow = useCallback(() => {
    window.webAgentAPI.openUrl(`craftagents://skills/skill/${skillSlug}?window=focused`)
  }, [skillSlug])
