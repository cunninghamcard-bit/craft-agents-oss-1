...
async function loadAutomationsFromServer(workspaceId: string): Promise<AutomationListItem[]> {
  const json = await window.webAgentAPI.getAutomations(workspaceId)
...
export function useAutomations(
  ...
  const loadAndHydrate = useCallback(async () => {
    ...
    try {
      ...
      try {
        const map = await window.webAgentAPI.getAutomationLastExecuted(activeWorkspaceId)
  ...
  useEffect(() => {
    if (!activeWorkspaceId) return
    const cleanup = window.webAgentAPI.onAutomationsChanged(() => { loadAndHydrate() })
    return () => { cleanup() }
  ...
  const handleTestAutomation = useCallback((automationId: string) => {
    ...
    window.webAgentAPI.testAutomation({
  ...
  const handleToggleAutomation = useCallback((automationId: string) => {
    const automation = findAutomation(automationId)
    if (!automation || !activeWorkspaceId) return
    window.webAgentAPI.setAutomationEnabled(
      activeWorkspaceId,
      automation.event,
  ...
  const handleDuplicateAutomation = useCallback((automationId: string) => {
    const automation = findAutomation(automationId)
    if (!automation || !activeWorkspaceId) return
    window.webAgentAPI.duplicateAutomation(activeWorkspaceId, automation.event, automation.matcherIndex)
  ...
  const confirmDeleteAutomation = useCallback(() => {
    ...
    window.webAgentAPI.deleteAutomation(activeWorkspaceId, pendingDeleteAutomation.event, pendingDeleteAutomation.matcherIndex)
  ...
  const getAutomationHistory = useCallback(async (automationId: string): Promise<ExecutionEntry[]> => {
    if (!activeWorkspaceId) return []
    try {
      const entries = await window.webAgentAPI.getAutomationHistory(activeWorkspaceId, automationId, 20)
      const automation = findAutomation(automationId)
  ...
  const handleReplayAutomation = useCallback((automationId: string, event: string) => {
    if (!activeWorkspaceId) return
    window.webAgentAPI.replayAutomation(activeWorkspaceId, automationId, event)
      .then(() => {
