...
export function useSessionMenuActions({
  ...
  const share = React.useCallback(async () => {
    ...
    if (result?.success && result.url) {
      ...
      toast.success(t('toast.linkCopied'), {
        ...
        action: {
          label: t('common.open'),
          onClick: () => window.webAgentAPI.openUrl(result.url!),
        },
      })
    } else {
  ...
  const showInFinder = React.useCallback(() => {
    window.webAgentAPI.sessionCommand(sessionId, { type: 'showInFinder' })
  }, [sessionId])

  const copyPath = React.useCallback(async () => {
    const result = await window.webAgentAPI.sessionCommand(sessionId, { type: 'copyPath' }) as { success: boolean; path?: string } | undefined
  ...
  const refreshTitle = React.useCallback(async () => {
    const result = await window.webAgentAPI.sessionCommand(sessionId, { type: 'refreshTitle' }) as { success: boolean; title?: string; error?: string } | undefined
    if (result?.success) {
  ...
  const openSharedInBrowser = React.useCallback(() => {
    if (!sharedUrl) return
    window.webAgentAPI.openUrl(sharedUrl)
  }, [sharedUrl])
  ...
  const updateShare = React.useCallback(async () => {
    const result = await window.webAgentAPI.sessionCommand(sessionId, { type: 'updateShare' })
  ...
  const revokeShare = React.useCallback(async () => {
    const result = await window.webAgentAPI.sessionCommand(sessionId, { type: 'revokeShare' })
    if (result && 'success' in result && result.success) {
