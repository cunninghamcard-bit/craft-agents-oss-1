...
export function useNotifications({
  ...
  const hasGuiChannels = useMemo(
    () => window.webAgentAPI.isChannelAvailable(RPC_CHANNELS.notification.SHOW),
    [],
  )
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onWindowFocusChange((isFocused) => {
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onNotificationNavigate((data) => {
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onBadgeDraw(async (data) => {
      try {
        const badgedIconDataUrl = await drawBadgeOnIcon(data.iconDataUrl, data.count)
        await window.webAgentAPI.setDockIconWithBadge(badgedIconDataUrl)
      } catch (error) {
    ...
    void window.webAgentAPI.refreshBadge()
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onBadgeDrawWindows(async (data) => {
      try {
        const overlayDataUrl = drawWindowsBadgeOverlay(data.count)
        await window.webAgentAPI.setDockIconWithBadge(overlayDataUrl)
      } catch (error) {
  ...
  const showSessionNotification = useCallback((session: Session, messagePreview?: string) => {
    ...
    window.webAgentAPI.showNotification(title, body, workspaceId, session.id)
