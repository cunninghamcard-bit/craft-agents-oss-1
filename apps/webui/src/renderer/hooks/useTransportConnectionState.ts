...
export function useTransportConnectionState(): TransportConnectionState | null {
  ...
  useEffect(() => {
    ...
    const readInitialState = async () => {
      if (!window.webAgentAPI.getTransportConnectionState) return
      try {
        const initial = await window.webAgentAPI.getTransportConnectionState()
        if (mounted) {
          setState(initial)
        }
      } catch {
    ...
    const unsubscribe = window.webAgentAPI.onTransportConnectionStateChanged?.((next) => {
      if (!mounted) return
