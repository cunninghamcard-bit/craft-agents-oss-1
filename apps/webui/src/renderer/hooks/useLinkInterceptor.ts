 ...
 *     └── can't preview? → webAgentAPI.openFile (opens externally)
 *
...
// ── Hook options ───────────────────────────────────────────────────────────────
// Callbacks injected by App.tsx so the hook doesn't depend on window.webAgentAPI directly.
...
export function useLinkInterceptor(options: LinkInterceptorOptions): LinkInterceptorResult {
  ...
  /** URLs always open externally; the WebUI does not embed a controlled webview. */
