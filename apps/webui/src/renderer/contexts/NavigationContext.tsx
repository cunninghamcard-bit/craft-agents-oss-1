...
export function NavigationProvider({
      ...
      switch (parsed.name) {
        case 'new-session': {
          ...
          if (parsed.params.name) {
            await window.webAgentAPI.sessionCommand(session.id, { type: 'rename', name: parsed.params.name })
          }
          ...
          if (parsed.params.status) {
            await window.webAgentAPI.sessionCommand(session.id, { type: 'setSessionStatus', state: parsed.params.status })
          }
          if (parsed.params.label) {
            await window.webAgentAPI.sessionCommand(session.id, { type: 'setLabels', labels: [parsed.params.label] })
          }
          ...
          if (parsed.params.input) {
            ...
            if (shouldSend) {
              setTimeout(() => {
                window.webAgentAPI.sendMessage(
                  session.id,
                  parsed.params.input!,
                  undefined,
                  undefined,
        ...
        case 'rename-session':
          if (parsed.id && parsed.params.name) {
            await window.webAgentAPI.sessionCommand(parsed.id, { type: 'rename', name: parsed.params.name })
          }
          break

        case 'delete-session':
          if (parsed.id) {
            await window.webAgentAPI.deleteSession(parsed.id)
          }
          break

        case 'flag-session':
          if (parsed.id) {
            await window.webAgentAPI.sessionCommand(parsed.id, { type: 'flag' })
          }
          break

        case 'unflag-session':
          if (parsed.id) {
            await window.webAgentAPI.sessionCommand(parsed.id, { type: 'unflag' })
          }
          break

        case 'oauth':
          if (parsed.id) {
            await window.webAgentAPI.performOAuth({ sourceSlug: parsed.id })
          }
          break

        case 'delete-source':
          if (parsed.id) {
            await window.webAgentAPI.deleteSource(workspaceId, parsed.id)
          }
          break
        ...
        case 'set-mode':
          if (parsed.id && parsed.params.mode) {
            ...
            await window.webAgentAPI.sessionCommand(
              parsed.id,
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onDeepLinkNavigate((nav: DeepLinkNavigation) => {
