...
export function ThemeProvider({
  ...
  useEffect(() => {
    ...
    window.webAgentAPI?.getColorTheme?.().then((configTheme) => {
  ...
  useEffect(() => {
    ...
    window.webAgentAPI?.getWorkspaceColorTheme?.(activeWorkspaceId).then((theme) => {
  ...
  useEffect(() => {
    ...
    // In playground/browser mode webAgentAPI may exist without loadPresetTheme.
    const loadPresetTheme = window.webAgentAPI?.loadPresetTheme
    if (!loadPresetTheme) {
      applyFallback(`webAgentAPI.loadPresetTheme is unavailable for "${effectiveColorTheme}".`)
      return () => {
        cancelled = true
      }
    }
  ...
  useEffect(() => {
    ...
    // Listen via WebUI IPC if available (more reliable on macOS)
    let cleanup: (() => void) | undefined
    if (window.webAgentAPI?.onSystemThemeChange) {
      cleanup = window.webAgentAPI.onSystemThemeChange((isDark) => {
        setSystemPreference(isDark ? 'dark' : 'light')
      })
    }

    // Fetch initial system theme from WebUI
    if (window.webAgentAPI?.getSystemTheme) {
      window.webAgentAPI.getSystemTheme().then((isDark) => {
        setSystemPreference(isDark ? 'dark' : 'light')
      })
    }
  ...
  useEffect(() => {
    if (!window.webAgentAPI?.onThemePreferencesChange) return

    const cleanup = window.webAgentAPI.onThemePreferencesChange((preferences) => {
  ...
  const setMode = useCallback((newMode: ThemeMode) => {
    ...
    if (!isExternalUpdate.current && window.webAgentAPI?.broadcastThemePreferences) {
      window.webAgentAPI.broadcastThemePreferences({ mode: newMode, colorTheme, font })
    }
  }, [colorTheme, font])
  ...
  const setColorTheme = useCallback((newTheme: string) => {
    ...
    if (!isExternalUpdate.current && window.webAgentAPI?.broadcastThemePreferences) {
      window.webAgentAPI.broadcastThemePreferences({ mode, colorTheme: newTheme, font })
    }
  }, [mode, font])
  ...
  const setFont = useCallback((newFont: FontFamily) => {
    ...
    if (!isExternalUpdate.current && window.webAgentAPI?.broadcastThemePreferences) {
      window.webAgentAPI.broadcastThemePreferences({ mode, colorTheme, font: newFont })
    }
  }, [mode, colorTheme])
  ...
  const setWorkspaceColorTheme = useCallback((newTheme: string | null) => {
    ...
    window.webAgentAPI?.setWorkspaceColorTheme?.(activeWorkspaceId, newTheme)
    // Broadcast to other windows
    window.webAgentAPI?.broadcastWorkspaceThemeChange?.(activeWorkspaceId, newTheme)
  }, [activeWorkspaceId])

  // Listen for workspace theme changes from other windows
  useEffect(() => {
    if (!window.webAgentAPI?.onWorkspaceThemeChange) return

    const cleanup = window.webAgentAPI.onWorkspaceThemeChange(({ workspaceId, themeId }) => {
