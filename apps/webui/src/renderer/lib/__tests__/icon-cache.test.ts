...
describe('null guard patterns', () => {
   ...
   *
   * ```ts
   * const result = await window.webAgentAPI.readWorkspaceImage(...)
   * if (!result) {
   *   return null
   * }
   * // Continue processing...
   ...
   *   const iconData = await window.webAgentAPI.readWorkspaceImage(...)
   *   if (!iconData) {
