...
export async function loadSourceIcon(
  ...
  try {
    const logoUrl = await window.webAgentAPI.getLogoUrl(serviceUrl, provider)
...
async function loadWorkspaceIcon(workspaceId: string, relativePath: string): Promise<string | null> {
  try {
    const result = await window.webAgentAPI.readWorkspaceImage(workspaceId, relativePath)
...
async function loadIconFile(
  ...
  try {
    const content = await window.webAgentAPI.readWorkspaceImage(workspaceId, relativePath)
    // IPC returns null for missing files (silent fallback)
    if (!content) {
      return null
    }
