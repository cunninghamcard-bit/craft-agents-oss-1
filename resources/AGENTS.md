...
This folder contains assets bundled with the web/headless service and synced to the user's `~/.craft-agent/` directory on launch.

## How It Works

1. **Build time**: server/Docker builds copy this folder into the deployable bundle
2. **Runtime**: `getBundledAssetsDir()` resolves paths to these bundled assets
3. **Launch**: Each asset type syncs to the user's home directory
...
These files are used by the service directly, not synced to user home:
