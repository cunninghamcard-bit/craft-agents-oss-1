...
4. **ALWAYS verify current API endpoints against current official docs or web sources** - URLs and docs change frequently
...
### 0.5. Choose Source vs One-Off Task Path (RECOMMENDED PRE-FLIGHT)
...
**Prefer a one-off task path first (or as fallback) when:**
...
If you choose the one-off path, still offer optional source setup later when the user needs repeatability.
...
Users often provide configs in Claude / Claude Code MCP format:
```json
{
  "mcpServers": {
    "airbnb": {
...
Create credentials at [Google Cloud Console](https://console.cloud.google.com/apis/credentials) and configure the OAuth callback URL used by the web/headless service.
...
2. **Choose the right path**: Confirm if this should be a source-backed workflow or a lighter local skill/automation
3. **Research before configuring**: Use WebSearch/WebFetch as needed
