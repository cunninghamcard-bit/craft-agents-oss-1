...
## 输出
> 面向非技术采购人员：输出只说查到什么、来源哪个平台/原厂，**不出现 CloakBrowser、浏览器、爬取、脚本、API、接口、cloak_search/api_search 等技术名词或工具名**；“没查到”分清两种：平台确实没这个型号→明说“XX 没有收录此型号”；有料但这次没取到→说清是“没取到不是没有”+业务化原因（需登录/访问受限/这次没连上），别用“暂时查不到”模糊带过。
